        ...
        case GGML_OP_SET_ROWS:
            {
                ...
                // turbo4/turbo2h block size is 128, so head_dim must be divisible by 128
                if ((op->type == GGML_TYPE_TURBO4_0 || op->type == GGML_TYPE_TURBO2H_0) && op->src[0]->ne[0] % 128 != 0) {
                    return false;
                }
