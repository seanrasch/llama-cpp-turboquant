...
static __global__ void k_set_rows_turbo3(
    ...
    // Butterfly stages: h=1..16 use warp shuffle (no barriers needed),
    // h=32..64 use shared memory (cross-warp communication required).
    {
        float v = x[j];

        // Intra-warp stages via __shfl_xor_sync — eliminates 5 __syncthreads() barriers
#define WHT_STAGE_SHUFFLE(h) \
        ...
        WHT_STAGE_SHUFFLE(4)
        WHT_STAGE_SHUFFLE(8)
        WHT_STAGE_SHUFFLE(16)
#undef WHT_STAGE_SHUFFLE

        x[j] = v;
    }
    __syncthreads();

    // Cross-warp stages via shared memory
...
static __global__ void k_set_rows_turbo2(
    ...
    // Intra-warp WHT stages via shuffle (no barriers)
    {
        float v = x[j];
#define WHT_STAGE_SHUFFLE_T2(h) \
        { float other = __shfl_xor_sync(0xffffffff, v, (h)); \
          v = (j & (h)) ? (other - v) : (v + other); }

        WHT_STAGE_SHUFFLE_T2(1)
        WHT_STAGE_SHUFFLE_T2(2)
        WHT_STAGE_SHUFFLE_T2(4)
        WHT_STAGE_SHUFFLE_T2(8)
        WHT_STAGE_SHUFFLE_T2(16)
#undef WHT_STAGE_SHUFFLE_T2
        x[j] = v;
    }
    __syncthreads();
...
static __global__ void k_set_rows_turbo4(
    ...
    {
        float v = x[j];
#define WHT_STAGE_SHUFFLE_T4(h) \
        { float other = __shfl_xor_sync(0xffffffff, v, (h)); \
          v = (j & (h)) ? (other - v) : (v + other); }

        WHT_STAGE_SHUFFLE_T4(1)
        WHT_STAGE_SHUFFLE_T4(2)
        WHT_STAGE_SHUFFLE_T4(4)
        WHT_STAGE_SHUFFLE_T4(8)
        WHT_STAGE_SHUFFLE_T4(16)
#undef WHT_STAGE_SHUFFLE_T4
        x[j] = v;
    }
    __syncthreads();
